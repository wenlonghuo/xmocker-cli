...
function validateArray(schema, names) {
	...
	if (schema.additionalItems !== undefined) {
		if (schema.additionalItems === false) {} else if (!isOfType(schema.additionalItems, 'object')) {
...
function validateObject(schema, names) {
	...
	if (schema.additionalProperties !== undefined) {
		if (schema.additionalProperties === false) {} else if (!isOfType(schema.additionalProperties, 'object')) {
			...
			switch (getType(schema.dependencies[property])) {
				...
				case 'array':
					for (var i = 0; i < schema.dependencies[property].length; ++i) {
						if (isOfType(schema.dependencies[property][i], 'string')) {} else {
