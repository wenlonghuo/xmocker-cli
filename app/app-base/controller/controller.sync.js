...
async function clientGetProjDiff (ctx, next) {
  ...
  try {
    ...
    return ctx.setError({ctx: ctx, next: next, err: '获取项目信息出错', e: e})
  }
...
async function clientGetApiDiff (ctx, next) {
  ...
  try {
    ...
    return ctx.setError({ctx: ctx, next: next, err: '获取api信息出错', e: e})
  }
  let host = await getServerInfo()
...
async function clientDownLoadProj (ctx, next) {
  ...
  if (!res.body || res.body.code) return ctx.setError({ctx: ctx, next: next, err: '下载项目信息出错'})
...
async function clientDownLoadApi (ctx, next) {
  ...
  let data = res.body.data
  if (!res.body || res.body.code) return ctx.setError({ctx: ctx, next: next, err: '下载api信息出错'})
...
async function serverGetProj (ctx, next) {
  ...
  try {
    ...
    return ctx.setError({ctx: ctx, next: next, err: '下载项目信息出错', e: e})
  }
  ctx.body = {
    code: 0,
    data: {
...
async function serverGetApi (ctx, next) {
  ...
  try {
    ...
    return ctx.setError({ctx: ctx, next: next, err: '下载api信息出错', e: e})
  }
...
async function serverDiffApi (ctx, next) {
  ...
  try {
    ...
    return ctx.setError({ctx: ctx, next: next, err: '下载api区别信息出错', e: e})
  }
...
async function serverDiffProj (ctx, next) {
  ...
  try {
    ...
    return ctx.setError({ctx: ctx, next: next, err: '下载项目区别信息出错', e: e})
  }
  ctx.body = {
    code: 0,
    data: result,
